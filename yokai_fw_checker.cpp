...
Node forward_step(const Node &node, unsigned char p) {
  ...
  A = static_cast<unsigned int>(prev.f8) + static_cast<unsigned int>(next.f5) +
      C;
  if (A > 0xff) {
    A &= 0xff;
    C = 1;
  ...
  A = static_cast<unsigned int>(p) + A + C1;
  if (A > 0xff) {
    A &= 0xff;
    C = 1;
  } else {
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < prefix_count; i++) {
    unsigned char p = prefix[i];
    auto nodes = forward_step_simd(start_node);
    for (int j = 0; j < NUM_CHARSET; j++) {
      unsigned char c = itoa[j];
      if (p == c) {
        start_node = nodes[j];
      }
    }
  ...
  printf("  %02X   %02X   %02X   %02X   %02X   %02X   %02X   %02X\n",
         start_node.digits.f4, start_node.digits.f5, start_node.digits.f6,
         start_node.digits.f7, start_node.digits.f8, start_node.digits.f9,
         start_node.digits.fa, start_node.digits.fb);
